...
import NumbersInput from "@/components/RegistTraining/numbersInput";
...
const TypeAndDurationInput: React.FC<TypeAndDurationInputProps> = ({
  ...
  return (
    <div className="grid grid-cols-2 gap-4 mt-12">
      <div>
        <Label htmlFor="training-type-select">Tipo de treino:</Label>
      ...
      <div>
        <Label htmlFor="training-duration-select">Duração do treino:</Label>
        <Select
...
export default function PinAndTrainingValidation({
  ...
  const handlePinValidated = async (pin: string) => {
    try {
      await apiPublic.post("/api/sms/verifyAlternativeTrainingOtp", {
  ...
  const handleSubmitTraining = async () => {
    ...
    console.log(trainingType.typeId);
    console.log(trainingDuration.durationId);

    if (pinCode.length === 4) {
      try {
        await apiPublic.post("/api/sms/registerAlternativeTraining", {
          otpCode: parseInt(pinCode, 10),
        ...
        setPin(["", "", "", ""]);
        setTrainingType({ typeId: "", typeName: "" });
        setTrainingDuration({ durationId: "", durationName: "" });
